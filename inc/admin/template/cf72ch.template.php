...
$use_2checkout					= intval( get_post_meta( $post_id, CF72CH_META_PREFIX . 'use_2checkout', true ) );
$debug_2checkout				= intval( get_post_meta( $post_id, CF72CH_META_PREFIX . 'debug', true ) );
$payment_mode_val				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'payment_mode', true ) );
$merchant_code					= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'merchant_code', true ) );
$secret_key						= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'secret_key', true ) );
$two_checkout_order_name		= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'two_checkout_order_name', true ) );
$amount							= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'amount', true ) );
$quantity						= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'quantity', true ) );
$success_returnurl				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'success_returnurl', true ) );
$cancel_returnurl				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'cancel_returnurl', true ) );
$currency						= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'currency', true ) );
$customer_email					= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'customer_email', true ) );

$billing_first_name				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'billing_first_name', true ) );
$billing_last_name				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'billing_last_name', true ) );
$billing_address				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'billing_address', true ) );
$billing_city					= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'billing_city', true ) );
$billing_state					= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'billing_state', true ) );
$billing_country				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'billing_country', true ) );
$billing_zipcode				= sanitize_text_field( get_post_meta( $post_id, CF72CH_META_PREFIX . 'billing_zipcode', true ) );
			...
			<div class="modal-content">
				<div class="inner-modal">
					...
					<div class="modal-footer">
						<a href="https://wordpress.org/support/plugin/accept-2checkout-payments-using-contact-form-7/reviews/" class="button primary-button review-cf72ch" target="_blank">' . __('Review us', 'accept-2checkout-payments-using-contact-form-7'). '</a>
						<button type="button" class="btn btn-default remind-cf72ch" data-dismiss="modal">'. __('Remind Me Later', 'accept-2checkout-payments-using-contact-form-7').'</button>
					</div>
					<div class="bird-icon">
		...
		'<table class="form-table">' .
			'<tbody>';
				...
				if( empty( $tags ) ) {
					...
					echo '<tr class="form-field">' .
						'<th scope="row">' .
							'<label for="' . CF72CH_META_PREFIX . 'use_2checkout">' .
								__( 'Enable/Disable', 'accept-2checkout-payments-using-contact-form-7' ) .
							'</label>' .
						'</th>' .
						'<td>' .
							'<input id="' . CF72CH_META_PREFIX . 'use_2checkout" name="' . CF72CH_META_PREFIX . 'use_2checkout" type="checkbox" class="enable_required" value="1" ' . checked( $use_2checkout, 1, false ) . '/>' .
						'</td>' .
					'</tr>' .
					'<tr class="form-field">' .
						'<th scope="row">' .
							'<label for="' . CF72CH_META_PREFIX . 'debug">' .
								__( 'Enable Debug Mode', 'accept-2checkout-payments-using-contact-form-7' ) .
							'</label>' .
						'</th>' .
						'<td>' .
							'<input id="' . CF72CH_META_PREFIX . 'debug" name="' . CF72CH_META_PREFIX . 'debug" type="checkbox" value="1" ' . checked( $debug_2checkout, 1, false ) . '/>' .
						'</td>' .
					'</tr>' .
					'<tr class="form-field">' .
						'<th scope="row">' .
							'<label for="' . CF72CH_META_PREFIX . 'payment_mode">' .
								__( 'Payment Mode ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
						'</th>' .
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'payment_mode" name="' . CF72CH_META_PREFIX . 'payment_mode" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>';
								if ( !empty( $payment_mode ) ) {
									foreach ( $payment_mode as $key => $value ) {
										echo '<option value="' . esc_attr( $key ) . '" ' . selected( $payment_mode_val, $key, false ) . '>' . esc_attr( $value ) . '</option>';
									}
								}
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'two_checkout_order_name">' .
								__( '2Checkout Order Item Name *', 'accept-2checkout-payments-using-contact-form-7' ) .
							'</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-order-name"></span>' .
						'</th>' .
						'<td>' .
							'<input id="' . CF72CH_META_PREFIX . 'two_checkout_order_name" name="' . CF72CH_META_PREFIX . 'two_checkout_order_name" type="text" class="large-text required-fields" value="' . esc_attr( $two_checkout_order_name ) . '" />' .
						'</td>' .
					'</tr>' .
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'merchant_code">' .
								__( 'Merchant Code ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .	
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-merchant-code"></span>' .
						'</th>' .
						'<td>' .
							'<input id="' . CF72CH_META_PREFIX . 'merchant_code" name="' . CF72CH_META_PREFIX . 'merchant_code" type="text" class="large-text required-fields" value="' . esc_attr( $merchant_code ) . '" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . ' />' .
						'</td>' .
					'</tr>' .
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'secret_key">' .
								__( 'Secret Key ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-secret-key"></span>' .
						'</th>' .
						'<td>' .
							'<input id="' . CF72CH_META_PREFIX . 'secret_key" name="' . CF72CH_META_PREFIX . 'secret_key" type="text" class="large-text required-fields" value="' . esc_attr( $secret_key ) . '" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . ' />' .
						'</td>' .
					'</tr>' .
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'amount">' .
								__( 'Amount Field Name ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-amount"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'amount" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'amount">' .
								'<option value="">' . __( 'Select field name for amount', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'quantity">' .
								__( 'Quantity Field Name (Optional)', 'accept-2checkout-payments-using-contact-form-7' ) .
							'</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-quantity"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'quantity" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'quantity">' .
								'<option>' . __( 'Select field name for quantity', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'customer_email">' .
								__( 'Customer Email ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-customer-email"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'customer_email" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'customer_email" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for customer email', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'currency">' .
								__( 'Select Currency', 'accept-2checkout-payments-using-contact-form-7' ) .
							' *</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-currency"></span>' .
						'</th>' .
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'currency" name="' . CF72CH_META_PREFIX . 'currency">';
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'success_returnurl">' .
								__( 'Return Success URL (Optional)', 'accept-2checkout-payments-using-contact-form-7' ) .
							'</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-success-returnurl"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'success_returnurl" name="' . CF72CH_META_PREFIX . 'success_returnurl">' .
								'<option>' . __( 'Select page', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'cancel_returnurl">' .
								__( 'Return Cancel URL (Optional)', 'accept-2checkout-payments-using-contact-form-7' ) .
							'</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-cancel-returnurl"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'cancel_returnurl" name="' . CF72CH_META_PREFIX . 'cancel_returnurl">' .
								'<option>' . __( 'Select page', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
					...
					echo '<tr class="form-field">' .
						'<th colspan="2">' .
							'<label for="' . CF72CH_META_PREFIX . 'customer_billing_details">' .
								'<h3 style="margin: 0;">' .
									__( 'Customer Billing Details', 'accept-2checkout-payments-using-contact-form-7' ) .
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'billing_first_name">' .
								__( 'Billing First Name Field ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-billing-first-name"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'billing_first_name" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'billing_first_name" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for billing first name', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'billing_last_name">' .
								__( 'Billing Last Name Field ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-billing-last-name"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'billing_last_name" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'billing_last_name" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for billing last name', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'billing_address">' .
								__( 'Billing Address Field ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-billing-address"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'billing_address" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'billing_address" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for billing address', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'billing_city">' .
								__( 'Billing City Field ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-billing-city"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'billing_city" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'billing_city" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for billing city name', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'billing_state">' .
								__( 'Billing State Field ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-billing-state"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'billing_state" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'billing_state" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for billing state name', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'billing_zipcode">' .
								__( 'Billing Zipcode Field ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-billing-zipcode"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'billing_zipcode" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'billing_zipcode" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for billing Zipcode name', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					...
					'<tr class="form-field">' .
						'<th>' .
							'<label for="' . CF72CH_META_PREFIX . 'billing_country">' .
								__( 'Billing Country Field ', 'accept-2checkout-payments-using-contact-form-7' ) .
							'*</label>' .
							'<span class="cf72ch-tooltip hide-if-no-js" id="cf72ch-billing-country"></span>' .
						'</th>'.
						'<td>' .
							'<select id="' . CF72CH_META_PREFIX . 'billing_country" class="cf72ch-required-fields" name="' . CF72CH_META_PREFIX . 'billing_country" ' . ( !empty( $use_2checkout ) ? 'required' : '' ) . '>' .
								'<option value="">' . __( 'Select field name for billing country name', 'accept-2checkout-payments-using-contact-form-7' ) . '</option>';
								if( !empty( $tags ) ) {
									foreach ( $tags as $key => $tag ) {
					 ...
					 * @var int $post_id
					 */
					do_action( CF72CH_PREFIX . 'add_fields', $post_id );
